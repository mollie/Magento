 ...
 */
class Mollie_Mpm_IdlControllerReturnActionTest extends MagentoPlugin_TestCase
{
	/**
	...
	public function setUp()
	{
		...
		$this->controller = $this->getMock("Mollie_Mpm_IdlController", array("getRequest", "_redirect", "loadLayout", "_showException"), array());
		...
		$this->datahelper  = $this->getMock("stdClass", array("getOrderIdByTransactionId", "getStatusById"));
	...
	public function testEverythingGoesGreat()
	{
		...
		$this->datahelper->expects($this->once())
			...
			'bank_status' => Mollie_Mpm_Model_Idl::IDL_SUCCESS,
		)));
	...
	public function testPaymentCancelled()
	{
		...
		$this->datahelper->expects($this->once())
			...
			->with(self::TRANSACTION_ID)
			->will($this->returnValue(array(
			'bank_status' => Mollie_Mpm_Model_Idl::IDL_CANCELLED,
		)));

		$this->controller->expects($this->once())
			->method("loadLayout")
			->will($this->throwException(new Test_Exception()));
